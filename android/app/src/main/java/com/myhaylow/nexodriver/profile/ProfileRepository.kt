...
interface ProfileConfigurationSource {
    fun profiles(): List<DriverProfile>
    fun activeProfileId(): String
    fun automaticEnabled(): Boolean
    fun schedules(): List<ProfileSchedule>
    fun costs(): OperatingCosts
}

class ProfileRepository(context: Context) : ProfileConfigurationSource {
    private val prefs = context.applicationContext.getSharedPreferences("nexo_local_settings", Context.MODE_PRIVATE)

    override fun profiles(): List<DriverProfile> {
    ...
    override fun activeProfileId() = prefs.getString("active_profile", DEFAULT.id) ?: DEFAULT.id
    fun activate(id: String) { if (profiles().any { it.id == id }) prefs.edit().putString("active_profile", id).apply() }
    override fun automaticEnabled() = prefs.getBoolean("schedule_enabled", false)
    fun setAutomaticEnabled(value: Boolean) { prefs.edit().putBoolean("schedule_enabled", value).apply() }

    override fun schedules(): List<ProfileSchedule> = prefs.getStringSet("schedules", emptySet()).orEmpty().mapNotNull(::decodeSchedule)
    ...
    override fun costs() = OperatingCosts(
