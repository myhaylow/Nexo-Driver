...
class MainActivity : Activity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        repository = ProfileRepository(this); RuntimeConfiguration.initialize(repository)
    ...
    private fun showSchedule() {
        ...
        repository.schedules().sortedBy { it.id }.forEach { schedule ->
            ...
            addView(label("$name • ${schedule.start}–${schedule.end} • prioridade ${schedule.priority}", 15f))
            addView(button("Excluir faixa") { repository.deleteSchedule(schedule.id); showSchedule() })
        }
        addView(button("Adicionar faixa (todos os dias)") { scheduleDialog() })
        }
    }
    ...
    private fun scheduleDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Nova faixa").setView(box).setNegativeButton("Cancelar", null).setPositiveButton("Salvar") { _, _ ->
            runCatching { repository.saveSchedule(ProfileSchedule(UUID.randomUUID().toString(), profiles[selected].id, DayOfWeek.entries.toSet(),
                LocalTime.parse(fields.second[0].text), LocalTime.parse(fields.second[1].text), fields.second[2].text.toString().toInt())) }
                .onFailure { toast("Use horários HH:mm e prioridade inteira") }; showSchedule()
        }.show()
    }
